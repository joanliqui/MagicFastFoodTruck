...
public class NetGazeInteractor : MonoBehaviour
{
    ...
    private NetGazeReticle _reticle;
    private NetBaseGazeInteractable interactable;
    ...
    private void Start()
    {
        _reticle = Instantiate(_reticlePrefab).GetComponent<NetGazeReticle>();
    ...
    private void Update()
    {
        ...
        if(Physics.Raycast(ray,out hit, maxDetectionDistance)) //Miramos si choca con cualquier objeto
        {
            ...
            NetBaseGazeInteractable inter = hit.collider.transform.GetComponent<NetBaseGazeInteractable>();
