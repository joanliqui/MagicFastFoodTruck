...
public class NetInventory : MonoBehaviour
{
    ...
    public void AddToInventory(IPickeable pick, Vector3 scale, Quaternion rotation, Material originalMat)
    {
        ...
        if(o.transform.TryGetComponent<NetFoodTruckObject>(out NetFoodTruckObject bf))
        {
            bf.enabled = false;
        }
    }
    ...
    public NetBaseFood GetCurrentObjectAsFood()
    {
        if(_pickableObject != null)
        {
            if (_pickableObject.gameObject.TryGetComponent(out NetBaseFood food))
            {
                return food;
            }
            else
            {
                Debug.Log("El objeto del inventario no es comida NET");
                return null;
            }
        }
